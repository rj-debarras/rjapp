...
export default function StepSurface({ data, updateData, nextStep }: any) {
  const options = [
    { label: "Moins de 50 m²", val: 50 },
    { label: "50 à 100 m²", val: 100 },
    { label: "Plus de 100 m²", val: 150 }
  ];

  return (
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out">
      <div className="text-center mb-10">
        <h3 className="text-3xl font-display font-bold text-primary mb-3 text-balance leading-tight">Quelle est la surface à vider&nbsp;?</h3>
        <p className="text-outline text-lg">Une estimation nous aide à prévoir l'équipe adaptée.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {options.map((opt, i) => {
          const isSelected = data.estimated_volume_m2 === opt.val;
          return (
            <button
               key={i}
               onClick={() => {
                 updateData({ estimated_volume_m2: opt.val });
             ...
             >
               <Maximize2 className={`w-8 h-8 mb-4 ${isSelected ? 'text-tertiary' : 'text-primary'}`} />
